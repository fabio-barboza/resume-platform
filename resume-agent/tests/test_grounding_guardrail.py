...
class TestSendsTheModelBackToSearch:
    ...
    def test_answer_from_memory(self):
        """Turno novo não herda a busca do turno anterior (regra 10)."""
        result = _run(
            [
    ...
    def test_invented_chart(self):
        """Regra 17a em código: número de gráfico sem ferramenta é chute."""
        answer = 'Distribuição:\n\n```chart\n{"type": "bar", "data": []}\n```'
        result = _run(
    ...
    def test_handmade_resume_link(self):
        """Regra 13a-i em código: link montado à mão a partir de um ID."""
